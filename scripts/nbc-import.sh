...
generate_recipe() {
    ...
    local id name distribution rootfs_type
    ...
    # Write recipe (assets + metadata only — boot args come from adapter at generate time)
    cat > "$dest_dir/recipe.yaml" <<EOF
# Auto-generated by nbc-import
# Boot args are NOT stored here — they come from the adapter YAML.
# To change boot args, edit adapters/${adapter_name}.yaml and run: nbc generate
